...
using IVY.Domain.Models.Vnpay;
using IVY.Domain.Models.Vnpay.Utilities;
...
namespace VNPAY.NET
{
    public class Vnpay : IVnpay
    {
        ...
        public string GetPaymentUrl(PaymentRequest request)
        {
            ...
            {
                throw new ArgumentException("Không có phương thức giao dịch");
            }

            var helper = new PaymentHelper();
            helper.AddRequestData("vnp_Version", _version);
            helper.AddRequestData("vnp_Command", request.Command);
