...
const Add = ({showPopup,setProducts}) => {
  
  const categories=useFetch(`${hostNameHttp}/api/danh-muc`).data;
  const collections=useFetch(`${hostNameHttp}/api/bo-suu-tap`).data;
  ...
  const handleSubmit = async (e) => {
    ...
    if(Object.keys(er).length === 0){
      ...
      const result=await post(`${hostNameHttp}/api/san-pham/them-moi`,product)
